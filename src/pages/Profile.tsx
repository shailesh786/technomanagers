...
import { Bookmark, Heart, MessageCircle, Trash2, Pencil } from 'lucide-react';
            ...
            <div className="divide-y">
              {savedItems.map((item: any) => (
                <div key={item.id} className="py-4 flex items-start justify-between gap-3">
                  <Link to={`/questions/${item.question?.id}`} className="flex-1 min-w-0 hover:text-primary transition-colors">
          ...
          {likedItems.length === 0 ? (
            ...
            <div className="divide-y">
              {likedItems.map((item: any) => (
                <Link key={item.question_id} to={`/questions/${item.question?.id}`} className="block py-4 hover:text-primary transition-colors">
                  <p className="font-medium text-sm line-clamp-2">{item.question?.question_text}</p>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {item.question?.company?.map((c: string) => <Badge key={c} variant="secondary" className="text-[10px]">{c}</Badge>)}
                  </div>
            ...
            <div className="divide-y">
              {commentItems.map((item: any) => (
                <div key={item.id} className="py-4">
                  <Link to={`/questions/${item.question?.id}`} className="text-xs text-primary hover:underline font-medium">
