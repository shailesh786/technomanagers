  ...
  return (
    <div className="flex min-h-[calc(100vh-4rem)]">
      ...
      <aside className="w-56 border-r bg-muted/30 p-4 space-y-1 hidden md:flex flex-col">
        <Link to="/" className="flex items-center gap-2 mb-4 px-2 text-sm text-muted-foreground hover:text-foreground transition-colors">
              ...
              {isRename && (
                <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 text-xs space-y-2">
                  <p className="font-medium text-amber-700 dark:text-amber-400">
                    Renaming "{editing?.originalName}" → "{editing?.name.trim()}"
                  </p>
                  <p className="text-muted-foreground">
                    Existing questions tagged with the old name will still reference "{editing?.originalName}" unless you update them.
                  </p>
                        ...
                        {(counts[c.name] || 0) > 0 && (
                          <label className="flex items-center gap-2 text-sm cursor-pointer px-1">
                            <input
                              ...
                              className="h-3.5 w-3.5 accent-primary"
                            />
                            Also remove "{c.name}" from all questions
                          </label>
                        )}
                        <AlertDialogFooter>
                          <AlertDialogCancel>Cancel</AlertDialogCancel>
            ...
            <tbody>
              {services?.map((s) => (
                <tr key={s.id} className="border-t">
                  ...
                  <td className="p-3 hidden md:table-cell">₹{s.price?.toLocaleString('en-IN')}</td>
...
function CoachingForm({ service, onClose }: { service: CoachingService | null; onClose: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div><label className="text-sm font-medium block mb-1">Short Description</label><Input value={form.short_description} onChange={(e) => setForm({ ...form, short_description: e.target.value })} /></div>
      <div className="flex gap-4">
        <div className="flex-1"><label className="text-sm font-medium block mb-1">Price (₹)</label><Input type="number" value={form.price} onChange={(e) => setForm({ ...form, price: parseInt(e.target.value) || 0 })} /></div>
        <div className="flex-1"><label className="text-sm font-medium block mb-1">Original Price (₹)</label><Input type="number" value={form.original_price} onChange={(e) => setForm({ ...form, original_price: parseInt(e.target.value) || 0 })} /></div>
      </div>
      <div className="flex gap-4">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {events && events.length === 0 && (
                <tr><td colSpan={6} className="p-8 text-center text-muted-foreground">No events yet. Click "Add Event" to create one.</td></tr>
              )}
            </tbody>
          </table>
        </div>
                ...
                <tr key={c.id} className="border-t">
                  ...
                  <td className="px-4 py-2 text-right">
                    ...
                    <AlertDialog onOpenChange={(o) => { if (!o) setDeleteAlsoRemoveFromQuestions(false); }}>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          <AlertDialogTitle>Delete company "{c.name}"?</AlertDialogTitle>
                          <AlertDialogDescription>
                            {(countMap[c.name] || 0) === 0
