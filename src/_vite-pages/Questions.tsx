...
export default function Questions() {
  ...
  const updateParam = useCallback((key: string, value: string | string[]) => {
  ...
  const setSort = useCallback((v: string) => {
    setSearchParams(prev => {
      const next = new URLSearchParams(prev);
      if (v && v !== 'Hot') next.set('sort', v); else next.delete('sort');
      return next;
    }, { replace: true });
  }, [setSearchParams]);
  ...
  // Map new filter state to useQuestions params
