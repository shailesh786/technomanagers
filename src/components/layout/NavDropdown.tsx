            ...
            <div className="flex-1 space-y-4">
              <div className="space-y-1">
                ...
                {roles.map((r) => (
                  <Link
                    key={r.id}
                    to={`/questions?role=${encodeURIComponent(r.name)}`}
                    onClick={() => setOpen(false)}
            ...
            <div className="w-48 space-y-1">
              ...
              <div className="flex flex-wrap gap-2">
                {popularCompanies.map((c) => (
                  <Link
                    key={c.company_name}
                    to={`/questions?company=${encodeURIComponent(c.company_name)}`}
                    onClick={() => setOpen(false)}
...
export function CoursesDropdown({ active }: { active: boolean }) {
  ...
  // Ensure Program Management section header always shows
